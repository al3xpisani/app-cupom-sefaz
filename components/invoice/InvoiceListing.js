...
const getInvoices = async () => {
  ...
  let invoices = [];
  const querySnapshot = await getDocs(collection(db, "nota-fiscal"));
...
const ListInvoices = () => {
  ...
  useEffect(() => {
    getInvoices().then((item) => setInvoices(item))
  }, []);
  ...
  const getItem = (_data, index) => {
    if (index in _data) {
      return {
        ...
        creation_timestamp: _data[index].data_emissao.nanoseconds,
